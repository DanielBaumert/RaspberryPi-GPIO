...
public sealed class EnvironmentPi4 : EnvironmentBase
{
    ...
    public override uint GPIO_OFFSET => 0x00200000;
    public override uint MAX_GPIO_COUNT => 57;
    public override uint SET_OFFSET => 0x07;
    public override uint CLR_OFFSET => 0x0A;
    public override uint LEV_OFFSET => 0x0D;

    public override uint FSEL_REGISTER_METHODE_INPUT => 0b000u;
    public override uint FSEL_REGISTER_METHODE_OUTPUT => 0b001u;
    public override uint FSEL_REGISTER_METHODE_MASK => 0b111u;
   
    public override uint CalcFSELRegisterOffset(uint gpio)
    {
        return gpio / 10;
    }
    public override uint CalcFSELRegisterShift(uint gpio)
    {
        return (gpio % 10) * 3;
    }
    public override uint CalcLEVRegisterIndex(uint gpio)
    {
        return gpio >> 5;
    }
    public override uint CalcLEVRegisterMask(uint gpio)
    {
        return 1u << (int)(gpio & 0b11111u);
    }

    public override uint CalcClrRegisterIndex(uint gpio)
    {
        return gpio >> 5;
    }
    public override uint CalcClrRegisterValue(uint gpio)
    {
        return 1u << (int)(gpio & 0b11111u);
    }

    public override uint CalcSetRegisterIndex(uint gpio)
    {
        return gpio >> 5;
    }
    public override uint CalcSetRegisterValue(uint gpio)
    {
        return 1u << (int)(gpio & 0b11111u);
    }
